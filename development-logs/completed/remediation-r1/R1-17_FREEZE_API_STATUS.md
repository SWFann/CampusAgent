...
## R1-E 端点统计口径清理（2026-07-15）

### 整改内容

`MVP_ENDPOINT_TRACEABILITY.md` 顶部已声明正确的权威统计（68 个 MVP HTTP 端点、3 个内部端点、71 个总文档化端点、100% 覆盖率），但正文仍保留整改前的旧数据，造成当前状态与历史数据混淆。

### 变更内容

1. **统计说明章节重构**：
   - 新增"整改前历史状态（已废弃）"子章节，明确标注旧数据（41 个已文档化、27 个缺失、60.3%）为历史记录，仅用于审计追溯
   - 新增"当前权威统计"子章节，明确当前有效口径（68/3/71/0/100%）
   - 保留"62 的来源分析"但明确标注为历史分析

2. **结论章节更新**：
   - 删除"API_CONTRACT.md 需要补充 27 个缺失端点"等过时结论
   - 更新为当前结论：API_CONTRACT.md 已完整文档化 71 个端点，覆盖率 100%

3. **后续行动章节清理**：
   - 删除重复的"待完成"章节
   - 将所有已完成的端点补全任务归入"已完成"章节
   - 移除已完成的 Scene、Model Gateway、Admin API 补全任务的待办标记

4. **P0_P1_REMEDIATION_PLAN.md 口径统一**：
   - R1-06 任务名称：建立 62 端点对照清单 → 建立 68 端点对照清单
   - R1 退出条件：62 个端点全部可追踪 → 68 个端点全部可追踪
   - R4-03 验收项：62 个 MVP 端点均有完整契约 → 68 个 MVP 端点均有完整契约
...
|------|------|
| MVP HTTP 端点 | 68 |
| Model Gateway 内部端点 | 3 |
| API_CONTRACT.md 总文档化端点 | 71 |
| 未文档化 MVP HTTP 端点 | 0 |
| MVP HTTP 端点覆盖率 | 100% |

### 关联任务

- R1-06：端点对照清单已建立（68 个端点）
- R1-07～R1-13：所有端点补全任务已完成
- R1-18：Admin API 认证方式冻结

## R1-22 WebSocket 鉴权说明（历史状态标注）

本日志中提到的 `ws://localhost:8000/ws/v1?token=<access_token>` 为 R1-17 完成时的历史状态。
