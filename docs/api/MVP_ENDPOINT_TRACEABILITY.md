...
| 9 | `GET /api/v1/users/{user_id}/agent` | User | ✅ 已文档化 | |
...
| 17 | `PATCH /api/v1/organizations/{organization_id}/members/{user_id}` | Organization | ✅ 已文档化 | |
| 18 | `DELETE /api/v1/organizations/{organization_id}/members/{user_id}` | Organization | ✅ 已文档化 | |
| 19 | `POST /api/v1/organizations/{organization_id}/join` | Organization | ✅ 已文档化 | |
| 20 | `POST /api/v1/organizations/{organization_id}/leave` | Organization | ✅ 已文档化 | |
...
以下数据为整改前的历史记录，**不代表当前状态**：

- **MVP_SCOPE.md 早期版本**：曾记录 **62** 个 MVP 端点（早期统计口径）
- **API_CONTRACT.md 整改前**：仅文档化 **41** 个端点
- **覆盖率（整改前）**：41/68 = **60.3%**
- **缺失端点（整改前）**：**27** 个

> ⚠️ **以上数字为历史数据**，仅用于审计追溯。当前状态以本文档顶部统计表为准。
...
|------|------|
| MVP 唯一 HTTP 端点 | 68 |
| Model Gateway 内部端点 | 3（不计入 HTTP 端点统计） |
| API_CONTRACT.md 文档化端点总数 | 71 |
| 未文档化 MVP HTTP 端点 | 0 |
| 文档覆盖率（HTTP 端点） | 100% |
...
**最可能的原因**：
- **文档未及时更新**：MVP_SCOPE.md 早期版本的"62"可能是当时的总数
- **计数误差**：文档撰写时的统计可能与实际端点列表不一致
- **排除逻辑不同**：62 可能排除了某些端点（如 Admin 部分端点）
...
- MVP_SCOPE.md 已更新：**68** 个 MVP 端点（不含内部端点 3 个）
- API_CONTRACT.md 已完整文档化：**71** 个端点（68 个 MVP HTTP + 3 个内部）
- 端点覆盖率：**100%**（0 个未文档化端点）
...
- [x] **更新 MVP_SCOPE.md**：从"62"改为"68"（R1-06）
...
- [x] **补充 API_CONTRACT.md**：完成 Scene API 9 个端点契约（R1-11）
  - [x] `GET /api/v1/scenes` - 场景列表
  - [x] `GET /api/v1/scenes/{scene_key}` - 场景详情
  - [x] `GET /api/v1/scene-instances/{scene_instance_id}` - 场景实例详情
