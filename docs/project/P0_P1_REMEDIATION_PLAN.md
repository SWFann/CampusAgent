...
|---|---|---|---|---|
| [x] | R0-01 | 保存当前工作树清单 | 仓库根目录 | 将 `git status --short` 输出保存到整改日志 |
| [-] | R0-02 | 创建整改分支 | Git | 用户授权在当前 `main` 工作树本地收口提交，未单独创建整改分支 |
| [x] | R0-03 | 区分原始产物与整改产物 | P0/P1 文件 | 列出 Claude 已生成文件、已有文件和本次新增文件 |
| [x] | R0-04 | 检查敏感信息 | 全仓库 | 扫描密钥、Token、真实学号、真实聊天和数据库文件 |
| [x] | R0-05 | 确认换行与编码 | Markdown、YAML、Python、TS | 所有文本为 UTF-8，提交前 `git diff --check` 通过 |
| [x] | R0-06 | 建立整改记录 | `development-logs/` | 每项整改记录任务、修改文件、命令结果和提交哈希 |
| [x] | R0-07 | 提交原始 P0/P1 快照 | Git | 在不包含密钥的前提下形成可追溯基线提交 |
...
|---|---|---|---|---|
| [x] | R1-01 | 确认全局角色集合 | 在四角色和六角色方案中作出正式决定 | 只能存在一套 `GlobalRole` 枚举 |
| [x] | R1-02 | 记录角色变更 ADR | 新增角色模型 ADR 或修订未接受 ADR | 解释 Counselor、OrganizationAdmin 属于全局角色还是组织授权能力 |
| [x] | R1-03 | 同步角色文档 | 计划书、词汇表、权限矩阵、API 示例 | 所有文档角色名称完全一致 |
| [x] | R1-04 | 清理待讨论术语 | `DOMAIN_VOCABULARY.md` | Consent、Capsule 等术语已确认或明确延期，不再处于未决状态 |
| [x] | R1-05 | 建立枚举对照表 | 领域词汇表 | 中英文、数据库值、API 值一一对应 |
...
| 状态 | ID | 整改内容 | 具体操作 | 完成标准 |
|---|---|---|---|---|
| [x] | R1-06 | 建立 68 端点对照清单 | `MVP_SCOPE.md` vs `API_CONTRACT.md` | 每个 MVP 端点都有契约章节和唯一编号 |
| [x] | R1-07 | 补全 Directory API | search、tree、recommended | 请求、响应、权限、分页和隐私投影齐全 |
...
| [x] | R1-30 | 检查隐私失败关闭 | 授权、加密、隔离失败场景 | 明确拒绝执行，不公开降级 |
| [x] | R1-31 | 复核保留策略 | 原始提交、胶囊、评价、AgentRun、Audit、Memory | 所有文档 TTL 一致 |
...
**R1-30 完成摘要（2026-07-15）**：

- ✅ 建立 FC-001～FC-012 失败关闭场景矩阵（12 类场景：授权失败、授权撤销、解密失败、租户隔离、privacy_context 缺失、模型路由、Prompt/输出校验、WebSocket 认证订阅、幂等重放、边缘节点、审计日志、清理保留）
- ✅ 测试定义从 78 增加到 90（新增 FC-001～FC-012）
- ✅ defined=90；not_run=90；executed=0；passed=0
- ✅ planned=9；implemented=0；verified=0
- ✅ R1-31 已完成
- ✅ 未修改端点数量（MVP HTTP 68、internal 3、总文档化 71）
- ✅ 未修改业务代码
- ✅ 重构 THREAT_MODEL.md §4.3 为 R1-30 权威口径
- ✅ 新增 API_CONTRACT.md §1.6.4 point 5 失败关闭规则
- ✅ 新增 WEBSOCKET_CONTRACT.md §9.3 WebSocket 失败关闭规则
- ✅ 新增 DATA_FLOW.md §9 失败关闭点标注
- ✅ 新增 DATA_INVENTORY.md §12 P2/P3/P4 失败关闭要求
- ✅ 未提交、未推送，等待 Codex 审计

**R1-31 完成摘要（2026-07-15）**：

- ✅ 建立 RT-001～RT-010 保留策略测试矩阵（10 类场景：临时数据删除、胶囊删除、导出过期、AgentRun 保留、AuditLog 保留、Memory 级联删除、评价归档、Token 失效、审计导出、日志脱敏）
- ✅ 测试定义从 90 增加到 100（新增 RT-001～RT-010）
- ✅ defined=100；not_run=100；executed=0；passed=0
- ✅ AuditLog 保留期限统一为 180 天（修正 API_CONTRACT 30天/90天/永久三处冲突）
- ✅ 保留策略权威口径见 DATA_INVENTORY.md §13 数据保留策略矩阵
- ✅ 更新 THREAT_MODEL.md §5.1 威胁覆盖表加入 RT 测试
- ✅ 更新 PRIVACY_TEST_MATRIX.md 双向追踪矩阵加入 RT 测试，章节重新编号
- ✅ 更新 ADR-0005 保留期限为 180 天
- ✅ 未修改端点数量（MVP HTTP 68、internal 3、总文档化 71）
- ✅ 未修改业务代码
- ✅ 未提交、未推送，等待 Codex 审计
...
|---|---|---|---|---|
| [x] | R1-32 | 修复全部内部链接 | P0 文档和开发日志 | 链接检查结果为 0 个失效链接 |
| [x] | R1-33 | 修正 P0 完成总结 | `P0_COMPLETION_SUMMARY.md` | 不再虚报 62 个契约、14 个威胁或已实施控制 |
| [x] | R1-34 | 更新 P0 进度表 | `DEVELOPMENT_PLAN.md` | 任务状态、阶段状态、日期和提交哈希一致 |
| [x] | R1-35 | 进行 P0 人工评审 | 全部 P0 文档 | 记录评审人、日期、决议和未决项 |
| [x] | R1-36 | 形成 P0 冻结提交 | Git | 一个或多个聚焦提交，CI 文档检查通过 |

### R1-32 完成摘要

- **检查范围**：`docs/` 全部 `.md` 文件（151 个文件）和 `development-logs/` 全部 `.md` 文件
- **修复链接数量**：9 个失效 Markdown 链接全部修复；另修复 3 处已归档日志旧路径引用
- **修复章节引用数量**：0（当前正文章节引用均已正确，历史变更记录中 §13 描述保留为历史记录）
- **当前失效链接数量**：0
- **R1-30/R1-31 日志已归档**：从 `development-logs/in-progress/` 移动到 `development-logs/completed/remediation-r1/`，旧版文件标记为 historical
- **未修改业务代码**
- **执行时未执行 R1-33～R1-36**
- **当前 R1-32 日志路径**：`development-logs/completed/remediation-r1/R1-32-fix-internal-links.md`

### R1-33 完成摘要

- **修改文件**：`docs/project/P0_COMPLETION_SUMMARY.md`
- **修正内容**：API 端点口径从「41/68 已文档化（60.3%）」改为「68 MVP + 3 internal = 71 总文档化端点（v1.0-frozen）」；M1 里程碑状态从「已验收」改为「附条件通过，R1 整改进行中」；P0 评审从「通过」改为「附条件通过」；追踪矩阵补充 §14 章节引用
- **未修改业务代码**
- **执行时未执行 R1-34～R1-36**
- **当前 R1-33 日志路径**：`development-logs/completed/remediation-r1/R1-33-correct-p0-completion-summary.md`

### R1-34 完成摘要

- **修改文件**：`docs/development/DEVELOPMENT_PLAN.md`、`docs/project/P0_P1_REMEDIATION_PLAN.md`
- **修正内容**：
  - 进度记录表（第 6 节）P0 从“未开始”改为“附条件通过（R1 整改收口中）”，P1 从“未开始”改为“工程底座已建立（待 R4 门禁）”，补充开始/完成日期、关键提交 `3f2ee03` 与备注
  - P0 阶段完成日期标注为“附条件通过，R1 整改收口中”，并新增当前阶段状态说明：契约 `v1.0-frozen`、71 端点、威胁 9 个、控制 `planned=9/implemented=0/verified=0`、测试 `defined=100/not_run=100`
  - P1 阶段新增当前阶段状态说明：P0/P1 仍待 R4 最终门禁，不得进入 P2
  - 新增第 6.1 节 R1 整改进度表：R1-32/R1-33/R1-34 已完成，R1-35/R1-36 未执行
  - 第 8 节补充状态说明，明确 P0/P1 已附条件通过并进入 R1 整改收口阶段
- **明确 R1-32/R1-33/R1-34 已完成**
- **R1-34 执行时 R1-35/R1-36 未执行；当前已由 R1-35/R1-36 收口**
- **未修改业务代码**
- **未修改 API/WebSocket 契约语义**
- **未改变测试定义数量（100）、威胁数量（9）、风险等级、控制状态（planned=9）**
- **当前 R1-34 日志路径**：`development-logs/completed/remediation-r1/R1-34-update-p0-progress-table.md`

### R1-35 完成摘要

- **修改文件**：`docs/project/P0_REVIEW_RECORD.md`、`docs/project/P0_P1_REMEDIATION_PLAN.md`
- **复审日期**：2026-07-16
...
| [x] | R2-06 | 验证全新安装 | 删除临时测试环境后重建或使用干净环境演练 | 单条文档命令可完成安装 |
...
| [x] | R2-19 | 增加边界说明 | 每个模块 README 或公开 `__init__` | 明确公开接口和禁止依赖 |
...
| [x] | R3-03 | 验证 frozen install | `pnpm install --frozen-lockfile` | 全新检出可重复安装 |
...
| [x] | R3-11 | 修复 install | 同时安装 Node 和 CampusAgent Python 依赖 | 不再只安装前端 |
...
| [x] | R3-16 | 延后 P2 命令 | Docker/Alembic 尚未存在 | 不在 P1 README 中声称当前可用 |
...
| [x] | R3-28 | 修复内部链接 | QUICK_START、TOOLING、开发日志 | 失效链接数量为 0 |
...
| [x] | R3-32 | 更新开发计划进度 | DEVELOPMENT_PLAN | P0/P1 状态、日期、提交/PR 一致 |
...
| 状态 | ID | 验收项 | 证据 |
|---|---|---|---|
| [x] | R4-01 | MVP/非 MVP 无歧义 | MVP Scope 评审记录 |
| [x] | R4-02 | 角色模型唯一且有 ADR | 角色 ADR + 全仓搜索结果 |
| [x] | R4-03 | 68 个 MVP 端点均有完整契约 | 端点对照表 |
| [x] | R4-04 | HTTP、Cookie、CSRF、WebSocket 认证一致 | ADR + API/WS 契约 |
| [x] | R4-05 | 状态机无未定义转换 | 状态转换矩阵评审 |
| [x] | R4-06 | 数据分类和保留期限一致 | 数据清单对照结果 |
| [x] | R4-07 | 威胁—控制—测试完整映射 | 可追踪矩阵 |
| [x] | R4-08 | P0 文档链接无失效 | 链接检查日志 |
| [x] | R4-09 | P0 所有未决项已处理 | 未决清单为空或有明确延期决议 |
| [x] | R4-10 | P0 评审通过 | 评审人、日期、提交哈希 |

### R4-B：P1 最终验收

| 状态 | ID | 验收项 | 证据 |
|---|---|---|---|
| [x] | R4-11 | CampusAgent Conda 环境可重建 | Python/依赖版本输出 |
