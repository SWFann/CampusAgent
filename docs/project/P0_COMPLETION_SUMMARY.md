...
- **内容**：100 个唯一测试定义（原有 51 + R1-29 新增 27 + R1-30 新增 12 FC + R1-31 新增 10 RT），定义状态 defined、执行状态 not_run，阻塞标准定义；威胁—控制—测试双向追踪矩阵；隐私失败关闭场景矩阵（FC-001～FC-012）；保留策略测试矩阵（RT-001～RT-010）
- **规模**：3,200+ 字
