...
- **内容**：9 个威胁：严重（Critical）1 个、高（High）6 个、中（Medium）2 个、低（Low）0 个；严重/高风险合计 7 个；9 个计划缓解策略，当前控制状态均为 `planned`，尚无 `implemented` 或 `verified` 控制
- **规模**：3,800+ 字

#### ✅ 隐私测试矩阵（P0-11）
- **文件**：`docs/privacy/PRIVACY_TEST_MATRIX.md`
- **内容**：100 个唯一测试定义（原有 51 + R1-29 新增 27 + R1-30 新增 12 FC + R1-31 新增 10 RT），定义状态 defined、执行状态 not_run，阻塞标准定义；威胁—控制—测试双向追踪矩阵（PRIVACY_TEST_MATRIX.md §14）；隐私失败关闭场景矩阵（FC-001～FC-012，§12）；保留策略测试矩阵（RT-001～RT-010，§13）
- **规模**：3,200+ 字
...
- **内容**：68 个 MVP 端点 + 3 个 internal 端点 = 71 个总文档化端点（v1.0-frozen），标准信封格式，统一错误码体系
- **规模**：3,500+ 字
...
通过隐私测试矩阵，明确了 **100 个唯一测试定义**（原有 51 + R1-29 新增 27 + R1-30 新增 12 FC + R1-31 新增 10 RT），包括：
- 25 个授权/拒绝测试（PT）
- 5 个场景隐私测试（ST）
- 5 个清理测试（CL）
- 9 个日志隐私测试（LG）
...
- 5 个模型路由测试（MR）
- 12 个边缘节点测试（EN）
- 12 个隐私失败关闭测试（FC，R1-30 新增）
- 10 个保留策略测试（RT，R1-31 新增）
- 定义状态：defined=100
- 执行状态：not_run=100
- 不代表测试已执行或通过
