...
## 17. P12 验证状态说明（2026-07-18 新增）

> **重要约束**：本节为 P12 阶段新增的验证状态说明，**不改变** §3~§13 中任何正式测试 ID 的定义状态（仍为 `defined`）和执行状态（仍为 `not_run`）。
>
> P12 新增的回归测试（`apps/api/tests/security/`、`apps/api/tests/integration/`、`apps/api/tests/performance/`）为部分正式测试 ID 提供了**等效验证证据**，但这些测试**不是**正式测试 ID 的官方执行记录。正式测试 ID 的执行状态升级需要按照 §15.1 测试时机要求，在对应阶段正式执行并记录。
>
> 本节的目的是**建立 P12 回归测试与正式测试 ID 之间的映射关系**，为后续正式执行提供参考。

### 17.1 P12 回归测试 → 正式测试 ID 映射

以下列出 P12 新增的回归测试文件及其等效覆盖的正式测试 ID。**等效覆盖不等于正式执行**。

| P12 测试文件 | 等效覆盖的正式测试 ID | 覆盖说明 |
| --- | --- | --- |
| `tests/security/test_auth_security.py` | RP-004、REV-004、RT-008（部分） | refresh token 轮换与重放、logout 后 token 失效 |
| `tests/security/test_csrf_and_cookies.py` | FC-008（部分） | CSRF 写请求强制、Cookie 安全属性 |
| `tests/security/test_idor_permissions.py` | PT-001~PT-005（等效）、PT-101~PT-104（等效）、PT-201~PT-204（等效）、PT-301~PT-306（等效）、FC-001（部分）、FC-004（部分） | 跨组织/会话/记忆 IDOR、admin 权限边界 |
| `tests/security/test_input_output_validation.py` | PI-004（部分）、FC-007（部分） | 输入校验、HTML/SQL 注入防御 |
| `tests/security/test_prompt_injection.py` | PI-001~PI-005（等效）、FC-005（部分）、FC-007（部分） | 私有字段不进入 prompt、输出 redaction |
| `tests/security/test_sensitive_redaction.py` | LG-001~LG-005（等效）、LG-101~LG-104（等效）、MR-005（部分）、EN-009（部分）、RT-010（部分） | 日志脱敏、metrics 安全、denylist 覆盖 |
| `tests/security/test_ttl_and_cleanup.py` | CL-001~CL-005（等效）、RT-001（部分）、RT-002（部分）、FC-012（部分） | 过期数据清理、TTL 边界 |
| `tests/integration/test_concurrency_idempotency.py` | RP-001~RP-003（等效）、RP-005（等效）、FC-009（部分） | 并发幂等、重复提交 |
| `tests/integration/test_websocket_stability.py` | FC-008（部分）、ST-001~ST-005（部分） | WebSocket 认证、订阅授权、非法消息 |
| `tests/security/test_observability_panel.py` | EN-009（部分）、MR-005（部分） | metrics 无 secret、admin 不泄露凭据 |
| `tests/performance/test_performance_budget.py` | — | 性能预算（无对应正式测试 ID） |
| `scripts/ops/recovery_drill.py` | RT-001（部分）、FC-006（部分）、FC-012（部分） | 恢复演练、降级行为 |

### 17.2 执行状态说明

- **正式测试 ID 执行状态**：仍为 `not_run`（全部 100 个）。P12 回归测试**不是**正式测试 ID 的官方执行。
- **P12 回归测试执行状态**：全部通过（后端 105 个新增测试通过，前端 9 个新增测试通过）。
- **升级条件**：正式测试 ID 的执行状态升级为 `passed` 需要：
  1. 按照 §15.1 测试时机在对应阶段正式执行。
  2. 测试结果记录到可追踪的测试报告中。
  3. 测试环境符合 §15.2 测试数据要求。
...
> 注：后端新增 105 个测试中，部分测试在 P12 前已存在但被增强。上表统计的是 P12 阶段新增或重大修改的测试。

### 17.4 未覆盖的正式测试 ID

以下正式测试 ID 在 P12 中**没有**等效回归测试覆盖，仍需要后续阶段正式执行：

- **EXP-001~EXP-003**（数据导出测试）：P12 未实现数据导出功能。
- **RT-003**（导出文件过期删除）：依赖数据导出功能。
- **RT-004**（AgentRun 保留 30 天）：需要长期运行环境。
- **RT-005**（AuditLog 保留 180 天）：需要长期运行环境。
- **RT-006**（Memory 级联删除）：P12 未覆盖用户/组织删除级联。
- **RT-007**（评价数据归档/删除）：P12 未实现评价归档。
- **RT-009**（审计日志导出）：依赖数据导出功能。
- **EN-002~EN-008、EN-010~EN-011**（边缘节点网络/凭据/隔离测试）：需要真实节点环境。
...
| 2026-07-18 | P12-14 隐私矩阵回填：新增 §17 P12 验证状态说明，建立 P12 回归测试与正式测试 ID 的映射关系；未修改任何正式测试 ID 的定义状态（仍为 defined）和执行状态（仍为 not_run）；P12 回归测试全部通过但不等同于正式测试 ID 的官方执行；相关文档从 §16 编号为 §17 | - |
